...
import {
  IoCloseOutline, IoCloudUploadOutline, IoChevronUpOutline, IoChevronDownOutline,
  IoTrashOutline, IoAddOutline,
...
const inputClass = 'w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-300 transition-all placeholder:text-slate-300 appearance-none';
const inlineInputClass = 'flex-1 px-3 py-2 bg-slate-50 border border-indigo-300 rounded-xl text-sm text-slate-800 outline-none focus:ring-2 focus:ring-indigo-200 transition-all placeholder:text-slate-400';
const addLinkClass = 'text-[10px] font-bold text-indigo-500 hover:text-indigo-700 flex items-center gap-0.5 transition-colors';
const inlineAddBtnClass = 'px-3 py-2 bg-indigo-500 hover:bg-indigo-600 disabled:bg-slate-100 disabled:text-slate-400 text-white text-xs font-bold rounded-xl transition-colors';
...
export const UploadModal: React.FC<UploadModalProps> = ({
  ...
  useEffect(() => {
    if (prefilledSectionId) { setSectionId(prefilledSectionId); return; }
  ...
  useEffect(() => {
    if (!isUploading) { setAnalyzeStepIndex(0); return; }
  ...
  const resolveTitleAndBody = async (extractedText: string, fallbackTitle: string): Promise<{ title: string; bodyText: string }> => {
    ...
    } catch { /* Keep filename fallback when title extraction is unavailable. */ }
  ...
  const moveUp = (idx: number) => {
    if (idx === 0) return;
    setImageFiles(prev => { const n = [...prev]; [n[idx - 1], n[idx]] = [n[idx], n[idx - 1]]; return n; });
  };
  ...
  const handleDragStart = (idx: number) => { dragIndexRef.current = idx; setDraggingIndex(idx); };
  ...
  const handleDragEnd = () => { dragIndexRef.current = null; setDraggingIndex(null); setDragOverIndex(null); };
  ...
  const filteredStudents = sectionId ? students.filter(s => s.sectionId === sectionId) : students;
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div
            className={`border-2 border-dashed rounded-2xl p-5 flex flex-col items-center gap-2 cursor-pointer transition-colors ${
              isDragging ? 'border-teal-400 bg-teal-50' : 'border-slate-200 hover:border-teal-300 bg-slate-50/60'
            }`}
          ...
          >
            <input
              ...
              accept=".txt,.pdf,.docx,image/*" multiple
              onChange={e => { const files = Array.from(e.target.files || []); if (files.length > 0) handleFilesSequentially(files); e.target.value = ''; }}
            />
            {isExtracting ? (
              <>
                ...
                <React.Fragment key={i}>
                  ...
                  <div
                    ...
                    className={`flex items-center gap-2 border rounded-xl px-3 py-2 cursor-grab active:cursor-grabbing transition-all ${
                      draggingIndex === i ? 'opacity-40 bg-slate-100 border-slate-300 scale-[0.98]' : 'bg-slate-50 border-slate-200'
                    }`}
                  >
                    <span className="w-5 text-[10px] font-bold text-slate-400 text-center shrink-0">{i + 1}</span>
                    <img
                    ...
                    />
                    <span onClick={() => setPreviewImage(f)} className="text-xs text-slate-700 flex-1 truncate cursor-pointer hover:text-teal-600">{f.name}</span>
                    <button onClick={e => { e.stopPropagation(); moveUp(i); }} disabled={i === 0} className="p-1 rounded text-slate-400 hover:text-slate-600 disabled:opacity-25 transition-colors">
                      <IoChevronUpOutline className="text-sm" />
                    </button>
                    <button onClick={e => { e.stopPropagation(); moveDown(i); }} disabled={i === imageFiles.length - 1} className="p-1 rounded text-slate-400 hover:text-slate-600 disabled:opacity-25 transition-colors">
                      <IoChevronDownOutline className="text-sm" />
                    </button>
                    <button onClick={e => { e.stopPropagation(); removeImage(i); }} className="p-1 rounded text-rose-400 hover:text-rose-600 transition-colors">
          ...
          {previewImage && (
            <div className="fixed inset-0 z-[60] bg-black/80 flex items-center justify-center p-4" onClick={() => setPreviewImage(null)}>
              <div className="relative max-w-3xl w-full" onClick={e => e.stopPropagation()}>
                <button onClick={() => setPreviewImage(null)} className="absolute -top-10 right-0 text-white/70 hover:text-white">
                  <IoCloseOutline className="text-3xl" />
                </button>
                <img src={`data:${previewImage.mimeType};base64,${previewImage.base64}`} alt={previewImage.name} className="w-full rounded-2xl shadow-2xl" />
