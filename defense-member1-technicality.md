...
2. **Text Complexity**: classifies a reading material or passage into one of three DepEd-aligned levels:
    - *Literal*: directly stated information that a Grade 7 learner can understand without inference
    - *Inferential*: meaning that must be read between the lines to judge whether it fits Grade 7 reading demand
    - *Evaluative*: judgment, critique, or author-purpose analysis that is more demanding than literal recall
...
- The dataset is small and domain-specific, so deep learning needs much larger data to outperform SVM on structured features.
...
The complexity model is trained on **64 Phil-IRI passages**: 24 Literal, 20 Inferential, and 20 Evaluative. These passages were professionally authored and labeled to specific complexity levels. Because they are the ground-truth standard, high in-sample accuracy is expected and correct. It means the model has learned the official standard. Generalization to new teacher-uploaded passages is evaluated separately.
...
1. Q: What is machine learning?
    A: Machine learning is how the SVM in ReadTrack learned to classify essays and passages. We trained it on 64 Phil-IRI passages and 1,145,716 ASAP2 essays so it can predict complexity and proficiency levels without being told the rules manually.
...
5. Q: What is training?
    A: Training means showing the SVM many labeled examples so it learns the boundaries between classes. For ReadTrack, we fed it 64 Phil-IRI passages labeled Literal, Inferential, or Evaluative and 1,145,716 ASAP2 essays with proficiency scores, then called SVC.fit() to build the model.
...
8. Q: What is a label?
    A: A label is the correct answer attached to each training sample. For complexity, labels are Literal, Inferential, or Evaluative, taken from Phil-IRI. For proficiency, the base labels come from the scored ASAP2 dataset, while teacher-rated essays in ReadTrack are added later as retraining data.

9. Q: What is classification?
    A: Classification means assigning input data to one of a fixed set of categories. ReadTrack classifies student essays into Nagsisimula, Papaunlad, or Mahusay, and reading passages into Literal, Inferential, or Evaluative, so the SVM picks one of three options each time.

10. Q: Why is ReadTrack a classification system?
    A: Both tasks, judging a student's reading proficiency and a passage's difficulty, map to fixed DepEd levels. The output is always one of three defined categories aligned to Phil-IRI and Grade 7 reading standards, not a continuous score.
...
12. Q: What are ReadTrack complexity labels?
    A: Literal, Inferential, and Evaluative are the three comprehension-demand levels used to judge Grade 7 reading materials. Literal is directly stated information, Inferential means reading between the lines, and Evaluative requires critical judgment.

13. Q: Why use NLP before ML?
    A: The SVM cannot work with raw text because it needs numbers. spaCy and CEFRpy convert each essay into 30 numeric values like sentence length, verb ratio, and CEFR word levels, which is the format the SVM needs to make a prediction.
...
17. Q: Why is POS tagging useful?
    A: POS tags let ReadTrack count patterns that separate reading levels. Evaluative texts tend to have more modal verbs (can, should, might) and abstract nouns than Literal texts, so spaCy's POS tags make those patterns countable as features.
...
40. Q: What is hyperparameter tuning?
    A: Hyperparameter tuning means searching for the best settings for the SVM before training. ReadTrack searches for the best C and gamma values, where C controls how strict the boundary is and gamma controls how much each training sample influences the boundary.
...
48. Q: What is underfitting?
    A: Underfitting means the model cannot separate the classes even on training data because it learns too little. ReadTrack avoids this through GridSearchCV, which selects C and gamma values that give the SVM enough flexibility to distinguish Literal from Inferential from Evaluative.
...
58. Q: Why is F1 important?
    A: A model that ignores Evaluative entirely could still show high accuracy if Literal is the most common class. F1 per class catches that because it shows whether the ReadTrack SVM handles all three levels fairly, not just the easy ones.
...
64. Q: Why use a confusion matrix?
    A: It reveals specific failure patterns that accuracy hides. For ReadTrack, the confusion matrix showed that Inferential is the hardest class because its features overlap more with Literal than Evaluative does, which guided us to add more discriminating features.
