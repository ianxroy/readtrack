...
| **Teacher-labeled samples** | Ongoing model improvement via Verify and Train | Ground truth from actual Grade 7 classroom context; most directly relevant to the target users |
...
| Label consistency | Phil-IRI labels come from published materials directly, not crowd annotators — removes inter-annotator disagreement |
...
The three complexity classes are approximately balanced in the Phil-IRI dataset, which reduces the risk of class bias.

---

## Possible Limitations — Honest Answers

- **Filipino NLP is less mature.** calamanCy provides POS tagging but CEFR features are unavailable for Filipino. Filipino essays have 6 fewer informative features than English essays.
...
84. Q: What is data leakage?
    A: Data leakage happens when information from the test set influences the model before evaluation. ReadTrack prevents this by fitting RobustScaler only on training data — the test set never influences the scaler's parameters.
...
86. Q: How is leakage prevented?
    A: ReadTrack's training scripts fit RobustScaler only on X_train. The same fitted scaler is then applied to X_test and saved to scaler.pkl for inference time — test data never influences the scaler's parameters.
...
89. Q: What is class imbalance?
    A: Class imbalance means one class has far more samples than the others, which can skew the model toward always predicting that class. The Phil-IRI dataset ReadTrack uses is approximately balanced across Literal, Inferential, and Evaluative, which reduces this risk.
